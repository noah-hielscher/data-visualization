class Info {
    constructor(trustData, crimeData, border, slider, sizeAll) {
        ...
        .forEach(trustObj => {
            ...
            .filter(crimeDataRowObj =>
                crimeDataRowObj["Country"] === trustObj["Countries"]
            ...
            this.data.push(this.countryInfo(trustObj, crimes, trustObj["Year"], score));
        });
    }
    ...
    searchHoveredCircleData() {
        const hovered = this.data.find(dataValue => {
            const distance = dist(mouseX, mouseY, dataValue.posX, dataValue.posY);
